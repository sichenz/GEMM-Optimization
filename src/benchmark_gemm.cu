...
#include "ops/op_mm_tensorcore_balanced.cuh"
...
// Benchmark balanced TensorCore GEMM (4 warps for better occupancy)
BenchmarkResult benchmarkTensorCoreBalancedGEMM(int M, int N, int K, int warmup_iters, int bench_iters) {
    Tensor<__half> A{M, K, true};
    Tensor<__half> B{K, N, true};
    Tensor<float> C{M, N, true};
    ...
    for (int i = 0; i < warmup_iters; i++) {
        op_mm_tensorcore_balanced(A, B, C);
    }
    CUDA_OK(cudaDeviceSynchronize());
    ...
    result.gflops = calculateGFLOPS(M, N, K, time_ms);
    result.bandwidth_gb_s = calculateBandwidth(M, N, K, time_ms, sizeof(__half));
    result.kernel_name = "Lab2_TensorCore_Balanced";
    result.dtype = "FP16";
...
// Benchmark 64×64 tile TensorCore GEMM (16 warps, larger tiles)
BenchmarkResult benchmarkTensorCore64x64GEMM(int M, int N, int K, int warmup_iters, int bench_iters) {
    Tensor<__half> A{M, K, true};
    Tensor<__half> B{K, N, true};
    Tensor<float> C{M, N, true};
    ...
    result.gflops = calculateGFLOPS(M, N, K, time_ms);
    result.bandwidth_gb_s = calculateBandwidth(M, N, K, time_ms, sizeof(__half));
    result.kernel_name = "Lab2_TensorCore_64x64";
    result.dtype = "FP16";
        ...
        if (prop.major >= 7) {
            ...
            */
            ...
            try {
                ...
                std::cerr << "Balanced TensorCore GEMM failed for M=" << M << " N=" << N << " K=" << K 
                          << ": " << e.what() << std::endl;
            } catch (...) {
                std::cerr << "Balanced TensorCore GEMM failed for M=" << M << " N=" << N << " K=" << K 
